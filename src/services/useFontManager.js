...
export function useFontManager() {
  const defaultFonts = [
    { id: 1, name: 'Roboto Mono', family: '"Roboto Mono", monospace', isDefault: true },
    { id: 2, name: 'Arial', family: 'Arial, sans-serif', isDefault: true },
    { id: 3, name: 'Georgia', family: 'Georgia, serif', isDefault: true },
  ];
  ...
  const removeFont = (id) => {
    const updatedFonts = fonts.filter((font) => font.id !== id || !font.isDefault);
    setFonts(updatedFonts);
    localStorage.setItem('fonts', JSON.stringify(updatedFonts));
    updateFontImports(updatedFonts);
  };
